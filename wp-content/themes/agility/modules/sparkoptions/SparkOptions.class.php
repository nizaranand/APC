...
class AgilityOptions{
	...
	function show(){
		...
		?>
		<!--<div class="sparkPanel-loading"></div>-->
		...
		switch($type){
			...
			case 'plugin':
				...
				if ( current_user_can( 'install_plugins' ) || current_user_can( 'update_plugins' ) ) {
					...
					$plugin_info = $this->getPluginInfo( $id );
	...
	function getPluginInfo( $id ){
		...
		$info = array();
		$trans_id = 'plugin_data_transient_'.$id;

		// Get any existing copy of our transient data
		if( false === ( $info = get_transient( $trans_id ) ) ) {
			// save the transient
			$info = plugins_api( 'plugin_information', array('slug' => $id ) );
			set_transient( $trans_id , $info , 60 * 60 * 24 );	//expires in 1 day
		}
