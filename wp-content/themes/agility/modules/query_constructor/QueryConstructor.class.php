...
class QueryConstructor{
	...
	function showShortcode(){
		...
		<div id="query_shortcode" class="tooltip" data-tooltip="<?php 
			...
			?>" >&#91;<?php echo $this->shortcode; ?> id=&quot;<span id="query_shortcode_id"><?php 
			echo $qid; ?></span>&quot;&#93;</div>
		<br/>
		<?php endif;
	}
...
$blogConstructor = new QueryConstructor( 'blogconstructor', 'Blog' , array( /*'shortcode' => 'blog'*/ ) );
...
class QueryStruct{
	...
	function specialSettings(){
		...
		unset( $this->settings['_wp_http_referer'] );
		unset( $this->settings['queryconstructor_wpnonce'] );
	}
...
class BlogQueryStruct extends QueryStruct{

	function __construct( $id = 0 ){

		parent::__construct( $id );
		$this->query_type = "Blog";
	}

	function loadDefaults(){	
		parent::loadDefaults();
		$this->defaults['post_type'] 	= 'post';	
		$this->defaults['order'] 		= 'DESC';
	}
	...
	function loadConfig(){
		...
		*/
		...
		$this->config['cat']['advanced'] = false;

		$this->config['posts_per_page']['title'] = __( 'Posts per page', 'agility' );
		$this->config['posts_per_page']['desc']	= __( 'This will set the maximum number of items per page in the blog.  Use -1 to show all matching items on one page.', 'agility' );
	...
	function specialSettings(){
		...
		$this->settings['paged'] = 	get_query_var( $paged_var );
				...
				<div id="qc_welcome_inner">
					...
					<ul class="tabs-content">
						<li id="tab-1" class="active">
							...
							<h2>Welcome to BlogConstructor!</h2>
							
							<p>You can use the BlogConstructor to create collections of portfolio items that form portfolios.  Here are some quick tips:</p>
