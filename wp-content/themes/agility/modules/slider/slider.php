...
$agility_slider_count = 0;

if ( ! function_exists( 'agility_slider' ) ) :
function agility_slider( $slider_id = 0, $config_type = 'slider_id', $slide_img_size = 'full' ){

	global $agilitySettings, $agility_slider_count;

	$slide_query = null;
	$sliderStruct = null;

	$agility_slider_count++;
	if( $slider_id == 0 ){
		$slider_id = $agility_slider_count;
	}
	...
	?>
				...
				<!-- FlexSlider -->
				<div id="slider-<?php echo $slider_id; ?>" class="flex-container flex-slider-count-<?php echo $agility_slider_count; ?>" data-slider-number="<?php echo $agility_slider_count; ?>" >
					...
					<?php if( $agilitySettings->op( 'slider-thumbnail-controller' ) ): ?>
					<!-- FlexSlider Carousel Controller -->
					<div class="flexcarousel-controller">
						<ul class="slides">
							<?php 
								$slide_query->rewind_posts();
								while ( $slide_query->have_posts() ) {
									echo '<li>';
									$slide_query->the_post();
									global $post;
									if( has_post_thumbnail() ){
										the_post_thumbnail( 'thumbnail' , array( 'class' => 'scale-with-grid size-thumbnail' ));
									}
									else if( $post->post_type == 'attachment' ){
										echo wp_get_attachment_image( get_the_ID() ); //, 'thumbnail', false, array( 'class' => 'scale-with-grid size-'.$slide_img_size ) );
									}
									echo '</li>';
								}
							?>
					...
					<!-- end FlexSlider Carousel Controller -->
					<?php endif; ?>
...
if ( ! function_exists( 'agility_item_carousel' ) ) :
function agility_item_carousel( $slider_id = 0, $config_type = 'slider_id', $slide_img_size = 'full' ){
	...
	$slide_count = $slide_query->post_count;
	$k = 0;
	?>
				<!-- FlexSlider -->
				<div class="flex-container">
					<div class="flexcarousel">
				    	...
				    	<?php while ( $slide_query->have_posts() ) {
				    			...
				    			switch( $k ){
				    				case 1:
				    					$GLOBALS['carousel_item_class'] = 'alpha';
				    					break;
				    				case $slide_count:
				    					$GLOBALS['carousel_item_class'] = 'omega';
				    					break;
				    				default:
				    					$GLOBALS['carousel_item_class'] = '';
				    					break;
				    			}
