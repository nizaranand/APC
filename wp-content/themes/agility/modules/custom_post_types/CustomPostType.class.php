...
abstract class AgilityCustomPostType {
	...
	public function __construct(){
		...
		do_action( 'agility_construct_'.$this->slug , $this );
...
class CustomMetaBox {
	...
	public function save($post_id){
		...
		if ($post->post_type == $this->page) {
			foreach($this->fields as $id => $field){
				$field->savePOST($post_id, $field->id);
			}
		}
		return $post_id;
...
abstract class MetaField {
	...
	public function savePOST( $post_id, $field_id ){
		$value;
		if( isset( $_POST[$field_id] ) ){
			$value = $_POST[$field_id];
		}
		else $value = $this->getDefault();

		$this->save( $post_id , $value );
	}
...
class CheckboxMetaField extends MetaField{
	...
	public function showField( $value = '' ){
		//global $post;
		//echo $post->ID;
		//echo get_post_meta( $post->ID, $this->id, true );
		?>
		<p>
	...
	public function savePOST( $post_id, $field_id ){
		...
		else $value = 'off';	//because it's a checkbox, we can't use a default when blank
