...
function agility_featured_image( $size = 'full', $class = '' , $expander = false, $gallery = '', $showCaption = false, $textOverImage = false ){
		...
		//If chosing to use thumbnails, set size to thumbnail
		if( agility_meta_on( 'custom_thumbnail' , 'off', $post->ID ) ) {
			$size = "thumbnail";
		}

		//$imagedata = wp_get_attachment_metadata( $post_thumbnail_id );
		//echo '<pre>'; print_r($imagedata); echo '</pre>';
		?>
		
		<!-- .featured-image ::agility_featured_image() -->
		<div class="featured-image img-wrapper <?php echo $class; ?> featured-image-size-<?php echo $size; ?>">
		...
		<?php elseif( !is_single() && !$agilitySettings->op( 'enable_lightbox' ) ): ?>
			...
			<a href="<?php the_permalink(); ?>" class="photo-link icon-link">
			<?php the_post_thumbnail( $size , array( 'class' => 'scale-with-grid')); ?>
			</a>
		...
		<?php elseif( get_post_meta( $post->ID , 'feature_type', true ) == 'video-embed' && $video_url ): ?>
			...
			<a href="#video-<?php echo $post->ID; ?>" data-rel="prettyPhoto<?php echo $gallery; ?>" class="video-link" title="<?php 
				echo strip_tags( get_the_excerpt() ); ?> <?php agility_ppLink_href(); ?>" <?php agility_alternative_href(); ?> ><?php 
		...
		<?php else: ?>
			...
			<a href="<?php echo $src[0] ?>" data-rel="prettyPhoto<?php echo $gallery; ?>" class="img-link preload" title="<?php 
				echo get_the_title( $post_thumbnail_id ); ?> <?php agility_ppLink_href(); ?>" <?php agility_alternative_href(); ?> ><?php 
 ...
 */
function agility_video_embed( $video_url , $flex = false , $width = 940 ){
	global $wp_embed;
	//echo $flex ? 'yes':'no';
	$class = '';
	if( strpos( $video_url , 'vimeo' ) !== false ) $class.= 'video-vimeo';
	if( strpos( $video_url , 'youtube' ) !== false ) $class.= 'video-youtube';
	
	/*if( $api ){
		if( strpos( $video_url , '?' ) !== false ) $video_url.='&api=1';
		else $video_url.= '?api=1';
	}*/

	?>	
						<div class="video-wrapper">
							<div class="video-container <?php if( $flex ) echo 'video-flex'; ?> <?php echo $class; ?>">
								<?php echo $wp_embed->run_shortcode( '[embed width="'.$width.'"]'.$video_url.'[/embed]' ); ?>
...
function agility_comment_form( $args = array(), $post_id = null ) {
	...
	$defaults = array(
		...
		'comment_field'        => '<p class="comment-form-comment"><label for="comment" class="fallback">' . _x( 'Comment', 'noun', 'agility' ) . '</label><textarea id="comment" placeholder="'._x( 'Comment', 'noun', 'agility' ).'" name="comment" cols="45" rows="8" aria-required="true"></textarea></p>',
 ...
 */
function agility_bloglist( $query=null, $cols='eight', $meta = true ){
...
function agility_social_media_icon( $site , $identifier, $title = '' ){
	...
	if( $agilitySettings->op( 'social-media-icon-source' ) == 'image' ):
	...
	?><a href="<?php echo $url; ?>" target="_blank" class="social-media tooltip-container" title="<?php echo $title; 
		?>" ><i class="tooltip-anchor icon fam-icon icon-<?php echo $site; ?>"></i><span class="tooltip"><?php echo $title; 
		?></span></a>
