 ...
 */
function agility_shortcode_accordion( $atts, $content ){

	global $shortcodeMaster;

	extract( shortcode_atts( $shortcodeMaster->getDefaults( 'accordion' ), $atts ) );
...
agility_add_shortcode( 'accordion', 'agility_shortcode_accordion' , array(
		'params' => array(),
		'content'	=> array(
			'title'	=> __( 'Toggles' , 'agility' ),
			'desc'	=> __( 'Wrap multiple [toggle]s in an [accordion] shortcode to make an accorion.' , 'agility' )
		)
	));
...
agility_add_shortcode( 'social_media' , 'agility_social_media_icons_shortcode', array(
		'params'	=> array(
			'order'	=> array(
				...
				'type' 	=> 'text',
				'default' => 'twitter, facebook, google-plus, forrst, dribbble, vimeo, youtube, linkedin, pinterest, flickr, tumblr'
			),
		),
		'content'	=> false,
	) ); // 'agility_social_media_icons' );
