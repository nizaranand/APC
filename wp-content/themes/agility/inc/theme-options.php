...
function agility_theme_options_init() {
	...
	$agilitySettings = new AgilityOptions( 
		...
		$agilitySettings->addTextInput( $basic, 
					'custom-sidebars-names',
					__( 'Custom Widget Area Names' , 'agility' ),
					__( '(Optional) Name your widget areas in a comma-delimited list.  e.g. <em>My First Widget Area,Special Widget Area</em>' , 'agility' ),
					''
					);
		...
		$agilitySettings->addSubHeader( $home,
			...
			__( 'Featured Carousel' , 'agility' ),
			__( '*New in v1.2* Normally this would be used in place of the above Featured Items, but you can use both if you wish' )
			);

		$agilitySettings->addCheckbox( $home,
			'home-show-carousel',
			__( 'Show Featured Carousel' , 'agility' ),
			'',
			'off'
		);
		...
		$agilitySettings->addSelect( $home,
			...
			__( 'Featured Carousel Slider' , 'agility' ),
			__( 'Select the slider to use on the Home page.  <a href="'.admin_url("admin.php?page=sliderconstructor").'" target="_blank">Create a new slider</a>' , 'agility' ), 
			'agility_slider_ops'
		);

		$agilitySettings->addCheckbox( $home,
			'home-featured-carousel-crop',
			__( 'Crop Featured Carousel Items' , 'agility' ),
			__( 'You can adjust the crop aspect ratio in the Images &amp; Videos tab' , 'agility' ),
			'off'
		);
		...
		$agilitySettings->addSelect( $blog,
			'blog-constructor-main-query',
			__( 'BlogConstructor Override' , 'agility' ),
			__( 'Select the blog to use for your main Blog Loop Query.  <a href="'.admin_url("admin.php?page=blogconstructor").'" target="_blank">Create a new blog</a>' , 'agility' ), 
			'agility_blog_ops'
		);
		...
		$agilitySettings->addSubHeader( $blog,
			'blog-loop-images',
			__( 'Blog Loop Image Cropping' , 'agility' )
			);

		$agilitySettings->addSelect( $blog,
			'blog-loop-standard-cropping',
			__( 'Standard Blog Cropping' , 'agility' ),
			__( 'Choose how to crop images in the Standard Blog Loop.  This can be overridden by custom thumbnails.' , 'agility' ), 
			array(
				'crop'		=>	'Crop - Uniformly cropped images',
				'natural'	=>	'Natural - Natural aspect ratios',
				'post'		=>	'Post Default - Use "Crop Featured Image" setting from post'
			),
			'post'
		);

		$agilitySettings->addSelect( $blog,
			'blog-loop-grid-cropping',
			__( 'Grid Blog Cropping' , 'agility' ),
			__( 'Choose how to crop images in the Grid Blog Loop.  This can be overridden by custom thumbnails.' , 'agility' ), 
			array(
				'crop'		=>	'Crop - Uniformly cropped images',
				'natural'	=>	'Natural - Natural aspect ratios',
				'post'		=>	'Post Default - Use "Crop Featured Image" setting from post'
		...
		$agilitySettings->addCheckbox( $images,
			'custom-thumbnail',
			__( 'Custom Thumbnails' , 'agility' ),
			__( 'Use a custom thumbnail for featured images instead of the default crops.  This is the default.  You can override it on each post.  Changing this will not affect old posts.' , 'agility' ),
			'off'
		);
		...
		$agilitySettings->addSelect( $social,
			'social-media-icon-source',
			__( 'Social Media Icons' , 'agility' ),
			__( 'Choose your social media icon source - images or fonts' , 'agility' ), 
			array(
				'image'	=>	'Images',
				'font'	=>	'Font',
			),
		...
		$agilitySettings->addTextInput( $social,
			'google-plus',
			'Google Plus', 
			__( 'Google Plus (Requires Font Icon usage)' , 'agility' )	//http://%.tumblr.com
		);

		$agilitySettings->addTextInput( $social,
			'forrst',
			'Forrst', 
			__( 'Forrst username' , 'agility' ) . ' **NOTE: not yet available when using font icons**'
		);

		$agilitySettings->addTextInput( $social,
			'dribbble',
			'Dribbble', 
			__( 'Dribbble username' , 'agility' ) . ' **NOTE: not yet available when using font icons**'
		);
		...
		$agilitySettings->addTextInput( $social,
			'tumblr',
			'Tumblr', 
			__( 'Tumblr URL or subdomain' , 'agility' )	
		);
		...
		$agilitySettings->addCheckbox( $slider,
			'slider-animateCaptions',
			__( 'Animate Captions' , 'agility' ),
			__( 'Slide caption in and out on slide change' , 'agility' ),
			'on'
		);

		$agilitySettings->addCheckbox( $slider,
			'slider-smoothHeight',
			__( 'Smooth Height' , 'agility' ),
			__( 'Animate the height of the slider as the height of the slides change.' , 'agility' ),
			'off'
		);

		$agilitySettings->addCheckbox( $slider,
			'slider-playPause',
			__( 'Play/Pause Button' , 'agility' ),
			__( 'Display the play/pause button.' , 'agility' ),
			'on'
		);

		$agilitySettings->addCheckbox( $slider,
			'slider-directionNav',
			__( 'Direction Navigation (Arrows)' , 'agility' ),
			__( 'Display the directional navigation arrows &lt; &gt;.' , 'agility' ),
			'on'
		);

		$agilitySettings->addSelect( $slider,
			'slider-controlNav',
			__( 'Control Navigation' , 'agility' ),
			__( 'Choose your slider control navigation' , 'agility' ), 
			array(
				'true'		=>	'Numbers',
		...
		$agilitySettings->addCheckbox( $slider,
			'slider-video',
			__( 'Allow Video' , 'agility' ),
			__( 'Allow video from YouTube and Vimeo to be embedded in the slideshow' , 'agility' ),
			'on'
		);

		$agilitySettings->addCheckbox( $slider,
			'slider-thumbnail-controller',
			__( 'Use Carousel Controller' , 'agility' ),
			__( 'Use a second slider of thumbnails to control the slider.  Note that this will override many of the settings above.  The thumbnail controller will use the featured image THUMBNAILS, which you should size appropriately.  That is, they will not be cropped by Agility, but via your WordPress thumbnail settings.' , 'agility' ),
			'off'
		);
		...
		$agilitySettings->addCheckbox( $misc,
			'raw_shortcode',
			__( '[raw] Shortcode' , 'agility' ),
			__( 'Enable use of the [raw] shortcode.  While this can make managing WordPress\' autoformatting easier, it can also break plugins.  For optimal compatibility, leave this off and use the "Disable Formatting Filters" on individual posts.' , 'agility' ),
			'off'
		);
		...
		$agilitySettings->addInfobox( $cust,
			'cust-notice-werkpress',
			__( 'Need Help With Advanced Customizations?' , 'agility' ),
			__( 'Sevenspark is partnered with WerkPress, a top notch WordPress customization shop.  You can '.
				'<a href="http://support.sevenspark.com/customizations/">contact them</a> and they\'ll be in touch '.
				'to discuss your project needs!' , 'agility' ),
		...
		$agilitySettings->addInfobox( $plugins,
			'plugin-cache-notice',
			__( 'Plugin Data is Cached for 24 Hours' , 'agility' ),
			__( 'To save loading time, plugin data is only refreshed once every 24 hours when you visit this page.' , 'agility' ),
			'spark-infobox-notice'
		);
...
/**
 * Retrieves saved blogs in proper options format
 */
function agility_blog_ops(){
	$bOps = QueryConstructor::getSavedQueries( 'Blog' );
	$ops = array( '0' => __( 'Default' , 'agility' ) );
	foreach( $bOps as $blog ){
		$ops[$blog->ID] = $blog->post_title;
	}
	return $ops;
}